...
def diag_lb2rt_flatten(height, width): # image_tensor
    pos2d = []
    count = 0 
    while count < width + height - 1:
        x = 0 if (count < height) else count - height + 1
        y = height - count - 1 if (count < height) else 0
        while (x <= width-1 and y <= height-1):
            ...
            y += 1
        count += 1
    pos2d =  torch.LongTensor(pos2d)
    return pos2d

def diag_rt2lb_flatten(height, width):
    pos2d = []
    count = 0 
    while count < width + height - 1:
        x = width - count - 1 if (count < width) else 0 
        y = 0 if (count < width) else count - width + 1 
        while (x <= width-1 and y <= height-1):
            pos_ij = [y, x]
            pos2d.append(pos_ij)
            x += 1
